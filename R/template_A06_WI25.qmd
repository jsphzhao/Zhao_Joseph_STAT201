...
cat("The largest win spread is", spread[max_spread_index], 
    ", when the Bears scored", bears[max_spread_index], 
    "points and the Opponent scored", opponent[max_spread_index], "points.\n")
```
...
```{r}
game_score <- data.frame(game_score)
game_score$win <- win
print(game_score)
```
...
```{r}
losses_before_first_win <- which(game_score$win)[1] - 1
...
rectangle_area <- function(length = 48, width = 48) {
  ...
  area_feet <- area_inches / (12^2)
  return(c(area_inches, area_feet))
}

output <- rectangle_area(102, 60)
cat("The area in square inches is", output[1], ".\n")
cat("The area in square feet is", output[2], ".\n")
```
...
```{r}
value_in_vector <- function(val, vec) {
  return(val %in% vec)
}
...
replace_negatives <- function(mat) {
  mat[mat < 0] <- NA
  return(mat)
}
...
```{r}
even_to_1_odd_to_0 <- function(mat) {
  mat <- ifelse(mat %% 2 == 0, 1, 0)
  return(matrix(mat, nrow = nrow(x)))
}

even_to_1_odd_to_0(x)
```
